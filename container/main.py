...
# Both application and MongoDB run on docker container then I just use the name of the service and no need to expose any port
...
# When MongoDB run in container but application run in my local machine then I have to expose the port 27017 so client(application) can send request to MongoDb
