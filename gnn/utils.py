...
__all__ = ['beamsearch', 'train_with_hyperparams', 'calculate_tour_cost', 'optimal_tour_as_tuples', 'calculate_distances', 'load_gnn_model']
...
def load_gnn_model(checkpoint_path,
                   device='auto',
                   **model_params):
...
def optimal_tour_as_tuples(optimal_tour: tuple[int]) -> tuple[tuple[int,int]]:
    optimal_tour = islice(cycle(optimal_tour),
                          len(optimal_tour) + 1)
