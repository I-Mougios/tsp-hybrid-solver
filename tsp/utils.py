...
__all__ = ['find_subtours', 'number_of_pairwise_distances', 'bool_','default', 'generate_clustered_tsp_data',
           'open_file', 'open_dataset', 'open_optimal_tour', 'match_dataset_with_optimal_solution',
           'match_tsplib_dataset_with_solution']

def find_subtours(solution_array: np.ndarray,
                  distances_df: pd.DataFrame) -> Tuple[Set[int]]:
    """
...
def bool_(value) -> bool:
    return False if math.isclose(value,
                                 0,
                                 rel_tol=1e-06,
                                 abs_tol=1e-06) else True
...
def match_dataset_with_optimal_solution(datasets_dir, optimal_solutions_dir):
    optimal_solutions_filenames = filter(lambda filename:
                                         filename.name.startswith('results'), optimal_solutions_dir.iterdir())
    ...
    for solution_path, dataset_path in zip(optimal_solutions_filenames, datasets_filenames):
        assert dataset_path.stem.split('_')[1:] == solution_path.stem.split('_')[
                                                   1:], f'Dataset and solution stem mismatch: {dataset_path.stem} and {solution_path.stem}'
...
def match_tsplib_dataset_with_solution(optimal_solutions_dir, datasets_dir):
    ...
    tsplib_datasets_solutions_paths = find_tsplib_dataset_based_on_solution_file(
                                                                        optimal_solutions_dir=optimal_solutions_dir,
