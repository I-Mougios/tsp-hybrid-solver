...
from tsp.utils import find_subtours, bool_, default
...
class TSP:
    ...
    def plot_optimal_tour(self, save_path=None,
                          style='fast',
                          color='black') -> None:
        plt.style.use(style)
        ...
        avg_y = np.mean(self.coordinates[:, 1])
        avg_x = np.mean(self.coordinates[:, 0])
        for i in range(len(self.optimal_tour) - 1):
            ...
            ax.annotate(
                text=f"{i + 1}", xy=(x_start, y_start), color=color,
            )

        # Plot the last arrow
        ax.arrow(x_end, y_end,
        ...
        ax.annotate(
            text=f"{i + 2}", xy=(x_end, y_end), color=color
        )
